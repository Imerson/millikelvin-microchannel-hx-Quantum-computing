# 1-Cu_0p5 -- OFHC copper, D_h = 0.5 mm

One of the six cases of the microchannel factorial (2 materials x 3 hydraulic diameters).
A OFHC copper cold plate at the mixing chamber of a dilution refrigerator holds a qubit chip
...
**Headline result: physical chip temperature = 59.19 mK** at the full 15 uW system load
(conservative upper bound), of which **70%** is the Kapitza interfacial jump.

| Parameter | Value |
|---|---|
| Material / kappa_s | OFHC copper / 13.08 W/(m K) (constant, see Assumptions) |
| Kapitza coefficient C_K | 0.020 K^4 m^2/W |
| Hydraulic diameter / wall | 0.5 mm / 0.25 mm |
| Channels in full 40x40 mm block | N_ch = 2809 (5-channel unit cell modelled, scaled by N_ch/5) |
| Load: system / modelled | 15 uW / Q_model = 26.70 nW |
| Flow | U = 1 mm/s, Re = 72 (laminar), inlet 10 mK |

## Results (converged, t = 6000)

| Quantity | Value |
|---|---|
| Gate-1 verified load | -26.70 nW (target -26.70, <0.1% error) |
| CFD conjugate interface temperature T_int | 25.00 mK |
| Kapitza resistance R_K = C_K/T_int^3 | 1280.5 m^2 K/W |
| Kapitza jump dT_K = (Q_model/A_wet) R_K | 34.19 mK |
| **Physical chip temperature T_chip = T_int + dT_K** | **59.19 mK** |
| Total resistance (T_chip - 10 mK)/Q_model | 1.842e+06 K/W |
| Kapitza fraction of the total rise | 70% |
| Pressure drop | 1.137e-02 Pa |
...
1. **CFD resolves the field problem** -- conjugate conduction, laminar flow, pressure drop,
   ...
   (grad = -5.1032e-06 K/m -> q'' = kappa x |grad| = 6.6750e-05 W/m^2, delivering exactly Q_model);
...
2. **The Kapitza jump is applied analytically** on the converged field:
   ...
   (A_wet = 1.000e-03 m^2 for the 5-channel cell). This is the standard treatment of a 0-D
...
1. **Constant solid conductivity** (kappa = 13.08 W/(m K), the Wiedemann-Franz value at
...
| Gate | Test | Result |
|---|---|---|
| 1 -- load | integral of wall heat flux over chip face = Q_model | pass (-26.70 nW, <0.1%) |
...
| 4 -- flow | laminar (Re = 72); dP = 1.137e-02 Pa | pass |
...
This case is the best copper case. Even at the finest machinable diameter, copper misses the 10-20 mK operating band by a factor of ~3 at the full 15 uW load: the Kapitza coefficient of the Cu-He interface is simply too large. Doubling the wetted area (vs Cu_1p0) buys a 37 mK improvement -- the area lever works -- but the material lever (see the silver cases) is worth a clean factor of 4 on the dominant term.

## How to open / re-run

- **ParaView**: open `Cu_0p5.foam`, load all regions (`solid_wall`, `domain1-5`), time 6000.
