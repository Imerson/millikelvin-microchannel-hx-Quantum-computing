# 2-Cu_1p0 -- OFHC copper, D_h = 1.0 mm
...
**Headline result: physical chip temperature = 96.00 mK** at the full 15 uW system load
(conservative upper bound), of which **83%** is the Kapitza interfacial jump.
...
| Kapitza jump dT_K = (Q_model/A_wet) R_K | 71.00 mK |
| **Physical chip temperature T_chip = T_int + dT_K** | **96.00 mK** |
| Total resistance (T_chip - 10 mK)/Q_model | 7.755e+05 K/W |
| Kapitza fraction of the total rise | 83% |
...
1. **CFD resolves the field problem** -- conjugate conduction, laminar flow, pressure drop,
   ...
   (grad = -1.0604e-05 K/m -> q'' = kappa x |grad| = 1.3870e-04 W/m^2, delivering exactly Q_model);
...
This case is the mid-range copper case and the reference geometry of the study (it is also the base case for the domain-independence verification). At 96 mK it sits far above the operating band; 83% of the chip-temperature rise is the interfacial (Kapitza) term, which no amount of copper machining can remove -- only area or material can.

## How to open / re-run

- **ParaView**: open `Cu_1p0.foam`, load all regions (`solid_wall`, `domain1-5`), time 6000.
