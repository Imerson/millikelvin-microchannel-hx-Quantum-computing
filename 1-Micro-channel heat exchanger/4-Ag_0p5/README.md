...
**Headline result: physical chip temperature = 33.54 mK** at the full 15 uW system load
(conservative upper bound), of which **36%** is the Kapitza interfacial jump.
...
| Kapitza jump dT_K = (Q_model/A_wet) R_K | 8.55 mK |
| **Physical chip temperature T_chip = T_int + dT_K** | **33.54 mK** |
| Total resistance (T_chip - 10 mK)/Q_model | 8.818e+05 K/W |
| Kapitza fraction of the total rise | 36% |
...
1. **CFD resolves the field problem** -- conjugate conduction, laminar flow, pressure drop,
   ...
   (grad = -7.2476e-06 K/m -> q'' = kappa x |grad| = 6.6750e-05 W/m^2, delivering exactly Q_model);
...
This case is the best case of the whole factorial: silver at the finest machinable diameter holds the chip at 33.5 mK under the full 15 uW load -- and this is a deliberate upper bound (see Caveats). Its system wetted area (0.56 m^2) is what the dissertation's minimum-area criterion identifies as just sufficient to hold the 10-20 mK band at low-microwatt loads. This is the recommended design geometry.

## How to open / re-run

- **ParaView**: open `Ag_0p5.foam`, load all regions (`solid_wall`, `domain1-5`), time 6000.
