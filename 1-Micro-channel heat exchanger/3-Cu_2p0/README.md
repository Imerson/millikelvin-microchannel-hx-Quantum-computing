# 3-Cu_2p0 -- OFHC copper, D_h = 2.0 mm
...
**Headline result: physical chip temperature = 167.08 mK** at the full 15 uW system load
(conservative upper bound), of which **90%** is the Kapitza interfacial jump.
...
| Kapitza resistance R_K = C_K/T_int^3 | 1280.6 m^2 K/W |
| Kapitza jump dT_K = (Q_model/A_wet) R_K | 142.09 mK |
| **Physical chip temperature T_chip = T_int + dT_K** | **167.08 mK** |
| Total resistance (T_chip - 10 mK)/Q_model | 3.540e+05 K/W |
| Kapitza fraction of the total rise | 90% |
...
1. **CFD resolves the field problem** -- conjugate conduction, laminar flow, pressure drop,
   ...
   (grad = -2.1208e-05 K/m -> q'' = kappa x |grad| = 2.7740e-04 W/m^2, delivering exactly Q_model);
...
This case is the coarsest geometry and the most Kapitza-dominated case of the factorial (90% of the temperature rise is interfacial). It illustrates the design rule in reverse: doubling the channel size halves the wetted area and roughly doubles the Kapitza jump, for a pressure-drop saving that was already negligible.

## How to open / re-run

- **ParaView**: open `Cu_2p0.foam`, load all regions (`solid_wall`, `domain1-5`), time 6000.
